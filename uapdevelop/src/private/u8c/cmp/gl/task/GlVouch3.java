...
public class GlVouch3 {
	public static VoucherResult MakeVouch(DJZBHeaderVO vo,ArrayList<DJZBItemVO> vobs
		...
		try{
			...
			Logger.debug("GlVouch3 vouchers:"+JSON.toJSONString(vouchers));
			Log.getInstance("cjbfTask").debug("GlVouch3 vouchers:"+JSON.toJSONString(vouchers));
			voucherResult=GlVouchAPIlink.invokeAPI(JSON.toJSONString(vouchers),dao);
		
		}catch(Exception e){
			strResult=e.getMessage();
			voucherResult.setMessage(strResult);
			Logger.error("GlVouch3:"+e.getMessage(),e);
			Log.getInstance("cjbfTask").error("GlVouch3:"+e.getMessage(),e);
			e.printStackTrace();
		}
