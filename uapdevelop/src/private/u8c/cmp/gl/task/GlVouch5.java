		...
		try{
			...
			*/
			...
			if (se!=0){
				...
				strExplanation=Explanation.getExplanation(deptdocVO.getDeptname(), scomment, custBasVO.getCustname(), vo.getZyx1(), "", 4);
			...
			Logger.debug("GlVouch5 vouchers:"+JSON.toJSONString(vouchers));
			Log.getInstance("cjbfTask").debug("GlVouch5 vouchers:"+JSON.toJSONString(vouchers));
			voucherResult=GlVouchAPIlink.invokeAPI(JSON.toJSONString(vouchers),dao);
		
		}catch(Exception e){
			strResult=e.getMessage();
			voucherResult.setMessage(strResult);
			Logger.error("GlVouch5:"+e.getMessage(),e);
			Log.getInstance("cjbfTask").error("GlVouch5:"+e.getMessage(),e);
			e.printStackTrace();
		}
